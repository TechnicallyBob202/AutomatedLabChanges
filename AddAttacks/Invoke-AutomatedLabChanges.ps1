...
Function Invoke-DomainAdminAction {
    ...
    switch ($domainAdminAction) {      
        ...
        #wmiFilterNew - creates a new WMI filter (msWMI-Som object) in AD
        wmiFilterNew {
            $wmiFilterNames = @(
                @{ Name = "Filter - Windows Server 2019";   Query = "SELECT * FROM Win32_OperatingSystem WHERE Version LIKE '10.0%' AND ProductType = 3 AND BuildNumber >= 17763" },
                @{ Name = "Filter - Windows Server 2022";   Query = "SELECT * FROM Win32_OperatingSystem WHERE Version LIKE '10.0%' AND ProductType = 3 AND BuildNumber >= 20348" },
                @{ Name = "Filter - Windows 11";            Query = "SELECT * FROM Win32_OperatingSystem WHERE Version LIKE '10.0%' AND ProductType = 1 AND BuildNumber >= 22000" },
                @{ Name = "Filter - Domain Controllers";    Query = "SELECT * FROM Win32_OperatingSystem WHERE ProductType = 2" },
                @{ Name = "Filter - Workstations Only";     Query = "SELECT * FROM Win32_OperatingSystem WHERE ProductType = 1" },
                @{ Name = "Filter - Hyper-V Hosts";         Query = "SELECT * FROM Win32_ComputerSystem WHERE HypervisorPresent = True" },
                @{ Name = "Filter - 64-bit Systems";        Query = "SELECT * FROM Win32_OperatingSystem WHERE OSArchitecture = '64-bit'" }
            )

            $wmiTarget = $wmiFilterNames | Get-Random
            $wmiName   = $wmiTarget.Name
            $wmiQuery  = $wmiTarget.Query

            try {
                $result = Invoke-Command -ComputerName $dcName -ErrorAction Stop -Credential $domainAdminCredential `
                    -ArgumentList $domainDN, $wmiName, $wmiQuery -ScriptBlock {
                        param($dn, $name, $query)
                        $msWMIPath = "CN=SOM,CN=WMIPolicy,CN=System,$dn"
                        $existing  = Get-ADObject -Filter { Name -eq $name } -SearchBase $msWMIPath -ErrorAction SilentlyContinue
                        ...
                        $msWMIParm2 = "1;3;10;$($query.Length);WQL;root\CIMv2;$query;"

                        $attrs = @{
                            "msWMI-Name"           = $name
                            "msWMI-Parm1"          = "$name - Created by AutomatedLabChanges"
                            "msWMI-Parm2"          = $msWMIParm2
                            "msWMI-Author"         = "AutomatedLabChanges@$($env:USERDNSDOMAIN)"
                        ...
                        try {
                            New-ADObject -Name $guid -Type "msWMI-Som" -Path $msWMIPath -OtherAttributes $attrs
                            return $true
                        } catch {
                            return $false
                        }
                ...
                switch ($result) {
                    $true    { if ($showAllActions) { Write-Host "      + created WMI filter: $wmiName" } }
                    "exists" { if ($showAllActions) { Write-Host "      ~ WMI filter already exists: $wmiName" } }
                    default  { Write-Host "      - could not create WMI filter: $wmiName" -ForegroundColor Red }
                }
            }
            catch {
                Write-Host "      - error creating WMI filter: $wmiName" -ForegroundColor Red
            }
        }

        #wmiFilterModify - updates the description (msWMI-Parm1) on an existing WMI filter
        wmiFilterModify {
            try {
                $result = Invoke-Command -ComputerName $dcName -ErrorAction Stop -Credential $domainAdminCredential `
                    -ArgumentList $domainDN -ScriptBlock {
                        param($dn)
                        $msWMIPath = "CN=SOM,CN=WMIPolicy,CN=System,$dn"
                        $filters   = Get-ADObject -Filter { ObjectClass -eq "msWMI-Som" } -SearchBase $msWMIPath `
                                        -Properties "msWMI-Name","msWMI-Parm1","msWMI-ChangeDate" -ErrorAction SilentlyContinue
                        if ($null -eq $filters) { return "none" }

                        $target = $filters | Get-Random
                        $now    = (Get-Date).ToUniversalTime().ToString("yyyyMMddHHmmss.ffffff+000")
                        $newDesc = "Modified by AutomatedLabChanges - $(Get-Date -Format 'yyyy-MM-dd HH:mm')"

                        try {
                            Set-ADObject -Identity $target.DistinguishedName -Replace @{
                                "msWMI-Parm1"      = $newDesc
                                "msWMI-ChangeDate" = $now
                            }
                            return $target."msWMI-Name"
                        } catch {
                            return $false
                        }
                    }
                ...
                switch ($result) {
                    "none"  { Write-Host "      ~ no WMI filters found to modify" -ForegroundColor Yellow }
                    $false  { Write-Host "      - could not modify WMI filter" -ForegroundColor Red }
                    default { if ($showAllActions) { Write-Host "      + modified WMI filter: $result" } }
                }
            }
            catch {
                Write-Host "      - error modifying WMI filter" -ForegroundColor Red
            }
        }

        #adSiteModify - updates the description/location on a random existing AD site
        adSiteModify {
            try {
                $result = Invoke-Command -ComputerName $dcName -ErrorAction Stop -Credential $domainAdminCredential -ScriptBlock {
                    $sites = Get-ADReplicationSite -Filter * -ErrorAction SilentlyContinue
                    if ($null -eq $sites) { return "none" }

                    $target = $sites | Get-Random
                    $newDesc = "Updated by AutomatedLabChanges - $(Get-Date -Format 'yyyy-MM-dd HH:mm')"
                    try {
                        Set-ADReplicationSite -Identity $target.DistinguishedName -Description $newDesc
                        return $target.Name
                    } catch {
                        return $false
                    }
                ...
                switch ($result) {
                    "none"  { Write-Host "      ~ no AD sites found to modify" -ForegroundColor Yellow }
                    $false  { Write-Host "      - could not modify AD site" -ForegroundColor Red }
                    default { if ($showAllActions) { Write-Host "      + modified AD site: $result" } }
                }
            }
            catch {
                Write-Host "      - error modifying AD site" -ForegroundColor Red
            }
        }

        #adSiteLinkModify - changes the cost or replication interval on a random AD site link
        adSiteLinkModify {
            try {
                $result = Invoke-Command -ComputerName $dcName -ErrorAction Stop -Credential $domainAdminCredential -ScriptBlock {
                    $links = Get-ADReplicationSiteLink -Filter * -ErrorAction SilentlyContinue
                    if ($null -eq $links) { return "none" }

                    $target      = $links | Get-Random
                    $newCost     = Get-Random -Minimum 50 -Maximum 500
                    $newInterval = @(15, 30, 60, 120, 180) | Get-Random
                    try {
                        Set-ADReplicationSiteLink -Identity $target.DistinguishedName -Cost $newCost -ReplicationFrequencyInMinutes $newInterval
                        return "$($target.Name) [cost=$newCost, interval=$newInterval]"
                    } catch {
                        return $false
                    }
                }
                ...
                switch ($result) {
                    "none"  { Write-Host "      ~ no AD site links found to modify" -ForegroundColor Yellow }
                    $false  { Write-Host "      - could not modify AD site link" -ForegroundColor Red }
                    default { if ($showAllActions) { Write-Host "      + modified AD site link: $result" } }
                }
            }
            catch {
                Write-Host "      - error modifying AD site link" -ForegroundColor Red
            }
        }
    }
}
...
# Set $rolesList['attack'] = 0 to disable this role entirely.
################################################################################
Function Invoke-AttackAction {
    ...
    switch ($attackAction) {
        ...
        #                toggles between insecure (1) and more secure (3) on each run
        #                visible as a risky security-relevant GPO change in DSP
        lmCompatLevel {
            $gpoSafeTargets = @("Servers - ALL - Blank", "Servers - ALL - Temporary")
            $gpoTarget = $null
            ...
            # Read current value to toggle it
            $regKey  = "HKLM\System\CurrentControlSet\Control\Lsa"
            $regName = "LmCompatibilityLevel"

            $currentVal = $null
            try {
                $currentVal = (Get-GPRegistryValue -Name $gpoTarget.DisplayName -Key $regKey -ValueName $regName -ErrorAction SilentlyContinue).Value
            }
            catch { }
...
#attack: set to 0 to disable entirely, 1 = fires roughly as often as domainadmin
$rolesList = @{}
...
#attack actions
# bruteForce    - repeated failed auth attempts against 1 random employee -> triggers account lockout + brute force IRP
# passwordSpray - 1 bad password attempted across 5 random employees       -> triggers password spray IRP
# lmCompatLevel - sets an insecure LmCompatibilityLevel in a lab GPO       -> visible as a risky GPO change
# note: target accounts are drawn from OU=Employees under the domainBaseLocation (defEmployees)
$attackActions = @{}
...
$domainAdminActions.Add('wmiFilterNew','2')
$domainAdminActions.Add('wmiFilterModify','2')
$domainAdminActions.Add('adSiteModify','2')
$domainAdminActions.Add('adSiteLinkModify','1')
